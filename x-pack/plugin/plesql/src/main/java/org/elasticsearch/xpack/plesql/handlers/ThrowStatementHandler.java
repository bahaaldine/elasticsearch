 ...
 * and throws a RuntimeException to propagate the error.
 */
public class ThrowStatementHandler {
    ...
    /**
     * Handles the THROW statement by extracting the exception message and throwing a RuntimeException.
     *
    ...
    public void handle(PlEsqlProcedureParser.Throw_statementContext ctx) {
        if (ctx.STRING() == null) {
            throw new RuntimeException("THROW statement requires a string message.");
        }
        ...
        // Throw a RuntimeException with the extracted message
